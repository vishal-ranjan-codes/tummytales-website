-- =====================================================
-- BELLYBOX - ADD PAYMENT METHOD TO CHECKOUT
-- Migration: 042_add_payment_method_to_checkout.sql
-- Description: Update bb_create_subscription_checkout to accept and store payment_method
-- =====================================================

-- Drop old function first (to avoid signature conflicts)
DROP FUNCTION IF EXISTS bb_create_subscription_checkout(UUID, UUID, DATE, UUID, UUID, JSONB);

-- Update bb_create_subscription_checkout to accept payment_method parameter
CREATE OR REPLACE FUNCTION bb_create_subscription_checkout(
    ...
    p_payment_method TEXT DEFAULT 'manual', -- NEW: payment method parameter
...
BEGIN
    ...
    IF p_payment_method IS NOT NULL AND p_payment_method NOT IN ('manual', 'upi_autopay') THEN
        RAISE EXCEPTION 'Invalid payment method: %. Must be "manual" or "upi_autopay"', p_payment_method;
    END IF;
    ...
    -- Create subscription group with payment_method
    INSERT INTO bb_subscription_groups (
        ...
        renewal_date,
        delivery_address_id,
        payment_method -- NEW: Store payment method
    ) VALUES (
        p_consumer_id,
        p_vendor_id,
        p_plan_id,
        ...
        v_renewal_date,
        p_address_id,
        COALESCE(p_payment_method, 'manual') -- NEW: Use payment method parameter, default to 'manual'
...
COMMENT ON FUNCTION bb_create_subscription_checkout IS 'Creates subscription group, subscriptions, cycle, and invoice. Accepts payment_method parameter (manual or upi_autopay). Uses SECURITY DEFINER to bypass RLS for system operations. Validates that consumer_id matches authenticated user.';
