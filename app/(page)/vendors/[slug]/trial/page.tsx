/**
 * Trial Page
 * Multi-step trial wizard for bb_* trials
 * Allows unauthenticated access - login happens inline in the wizard
 */

import { notFound } from 'next/navigation'
import { createClient } from '@/lib/supabase/server'
import TrialBuilder from '@/app/components/customer/TrialBuilder'
...
export default async function TrialPage({ params, searchParams }: PageProps) {
  const { slug } = await params
  const { step } = await searchParams

  const supabase = await createClient()

  // Optional authentication check - allow unauthenticated access
  const {
    data: { user },
