...
): Promise<ActionResponse<{ holidayId: string }>> {
  try {
    const supabase = await createClient()
    ...
    // Create holiday using service
    const { createVendorHoliday: createHolidayService } = await import('@/lib/services/vendor-service')
    const holidayId = await createHolidayService(supabase, {
      vendorId: vendor.id,
      date: data.date,
      slot: data.slot || null,
      reason: data.reason,
    ...
    revalidatePath('/dashboard/vendor/holidays')
    return { success: true, data: { holidayId } }
  } catch (error: unknown) {
    console.error('Error creating vendor holiday:', error)
    return { success: false, error: error instanceof Error ? error.message : 'Failed to create holiday' }
  }
}
 ...
 */
export async function deleteVendorHoliday(holidayId: string): Promise<ActionResponse> {
  try {
    const supabase = await createClient()
    ...
    // Verify holiday belongs to user's vendor
    ...
    interface HolidayWithVendor {
      vendors: { user_id: string } | { user_id: string }[]
    }
    const holidayWithVendor = holiday as unknown as HolidayWithVendor
    const vendorCheck = Array.isArray(holidayWithVendor?.vendors) 
      ? holidayWithVendor.vendors[0] 
      : holidayWithVendor?.vendors

    if (!holiday || !vendorCheck || vendorCheck.user_id !== user.id) {
      return { success: false, error: 'Holiday not found or access denied' }
    }
    ...
    return { success: false, error: error instanceof Error ? error.message : 'Failed to delete holiday' }
  }
}

/**
 * Get vendor holidays for a period
 */
export async function getVendorHolidays(
  ...
  try {
    ...
    console.error('Error getting vendor holidays:', error)
    return { success: false, error: error instanceof Error ? error.message : 'Failed to get holidays' }
  }
}
