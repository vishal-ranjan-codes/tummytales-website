-- =====================================================
-- BELLYBOX - UPDATE ORDER GENERATION WITH DELIVERY WINDOWS
-- Migration: 033_update_order_generation_with_delivery_windows.sql
-- Description: Update bb_finalize_invoice_paid to populate delivery windows in orders
-- =====================================================

-- First, add delivery window columns to bb_orders if not exists
...
-- Add comment
COMMENT ON COLUMN bb_orders.delivery_window_start IS 'Start time of delivery window for this order (TIME in IST)';
COMMENT ON COLUMN bb_orders.delivery_window_end IS 'End time of delivery window for this order (TIME in IST)';

-- Drop and recreate bb_finalize_invoice_paid with delivery window population
-- Note: This is a partial update - we'll read the full function and update just the order creation part

-- Create a helper function to get delivery windows for a vendor/slot
...
COMMENT ON FUNCTION bb_get_delivery_window IS 'Get delivery window for a vendor/slot combination with fallback defaults';

-- Now update bb_finalize_invoice_paid to use delivery windows
-- Drop all versions of the function
DROP FUNCTION IF EXISTS bb_finalize_invoice_paid(UUID, TEXT) CASCADE;
...
BEGIN
    ...
    LOOP
        ...
        -- Get delivery window for this subscription
